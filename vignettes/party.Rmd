...
$$\textrm{Partisan Dislocation(i)} = \textrm{voteshare}_D - \textrm{voteshare}_{KNN(i)}$$

where $KNN(i)$ denotes the k-nearest neighbors of a voter in precinct $i$. Note that for computational reasons and to match the inputs of other functions, we do not calculate random points for each voter. Instead, we use the distance between precincts to determine the k-nearest neighbors. 
To create a single score for a district, we can take the mean of the partisan dislocation scores for each voter in the district. Following Deford, Eubank, and Rodden 2022, we can also take the absolute value of the partisan dislocation scores to get the partisan dislocation score for a district.
